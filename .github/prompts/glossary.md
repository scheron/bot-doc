...
These already appear in English in the product interface. Reproduce them exactly,
including capitalization and spacing, and never translate them.

`Buy`, `Sell`, `Price`, `Quote`, `Orderbook`, `Ratio formula`, `Robot logs`,
`Portfolios table`, `Head of traders`, `Workspaces`, `Time (sec)`, `Enabled`,
`Disabled`, `Is first`, `Count`, `On buy`, `Type`, `Reset statuses`,
`shared memory`, `lot_size`, `LOCAL`, `_LOCAL`, `true`, `false`, `double`,
`call`, `put`, `rate`, `offer`, `WebSocket`, `Ed25519`
