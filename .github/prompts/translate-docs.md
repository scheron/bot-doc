...
# Fenced block placeholders

Fenced code blocks are removed before you see them and replaced with a line
of the exact form `[[FENCE_1]]`, `[[FENCE_2]]`, and so on. Copy every such
line into your output exactly as it appears, on its own line, in the same
order it appeared in the input. Do not add anything between two placeholder
lines, do not remove one, and do not translate, renumber, merge, split, or
