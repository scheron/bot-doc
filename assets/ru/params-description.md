...
Задает размер искусственного проскальзывания, определяя, на какое количество пунктов допустимо отступить от заданной цены в худшую сторону. Это отступ от рыночной цены, т.е. при покупке цена выставления `offer` + `k`, при продаже цена выставления `bid`−`k`, где `bid` и `offer` – лучшие цены на покупку и продажу, соответственно. Отображаемые (например, в виджете Portfolios table) цены [Price_s/Price_b](params-description.md#p.price_s) уже содержат параметр `k`малое. Коэффициент `k` используется при любом выставлении, за исключением только выставления по кликеру Place order. В случае выбора Type price: Orderbook для расчетов по второй ноге задаются параметры инструментов портфеля Calc price OB и Trading price OB. Коэффициенты k или ksl действуют после расчета с учетом этих параметров.  

### k_sl <Anchor :ids="['s.k_sl']" />

Аналог параметра `k`, но используется только во время перестановок заявок по [SLE](params-description.md#s.sle) и [TE](params-description.md#s.te). Задает размер искусственного проскальзывания, а именно отступ от рыночной цены, т.е. для заявки на покупку цена выставления `offer` + `k_sl`, для заявки на продажу цена выставления `bid`−`k_sl`, где `bid` и `offer` – лучшие цены на покупку и продажу, соответственно. Отображаемые (например, в виджете Portfolios table) цены [Price_s/Price_b](params-description.md#p.price_s) уже содержат параметр `k_sl`. Коэффициент `k_sl` используется при любом выставлении, за исключением только выставления по кликеру Place order. В случае выбора Type price: Orderbook для расчетов по второй ноге задаются параметры инструментов портфеля Calc price OB и Trading price OB. Коэффициенты k или ksl действуют после расчета с учетом этих параметров. 
