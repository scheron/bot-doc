...
Выставляет котирующие (т.е. при включенном режиме [Quote](params-description.md#p.quote)) заявки по [Is first](params-description.md#s.is_first) финансовому инструменту с признаком "снять если заявка будет taker" (при условии что биржей поддерживается работа этого типа заявки). Режим [Quote](params-description.md#p.quote) должен быть сначала выбран и применен кнопкой Apply, чтобы флаг [Only maker](params-description.md#p.maker) стал доступным.
