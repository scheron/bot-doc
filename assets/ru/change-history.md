...
- В C++ API добавлена возможность итерирования по финансовым инструментам портфеля (методы объекта [портфеля](c-api.md#доступ-и-изменение-полей-портфеля) `restart_sec_iter()`, `has_next_sec()`, `next_sec()`);
