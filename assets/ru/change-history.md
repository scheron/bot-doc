...
Добавлены параметры портфеля Shift mode, Shift formula, X, v_side, отвечающие за новые алгоритмы подвижки цен Lim_sell и Lim_buy;
В C++ интерфейс добавлены методы портфеля для доступа и редактирования Shift mode, X, v_side;
В C++ интерфейс добавлен метод deal() для объекта портфеля.
