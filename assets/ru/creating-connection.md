...
Для подключения необходимо создать пару ключей `Ed25519` c помощью утилиты.

`PUBLIC KEY` необходимо указать на самом Binance при добавлении ключа и получить готовое значение [API key](creating-connection.md#tc.BINANCE.ws_id).
...
Создать новые пары закрытого и открытого ключей `Ed25519`. Также можно вставить существующий закрытый ключ `Ed25519` в текстовое поле, и приложение автоматически сгенерирует соответствующий открытый ключ `Ed25519`.
...
Скопировать открытый ключ `Ed25519`, сгенерированный с помощью генератора асимметричных ключей, и вставить его в поле для регистрации.
