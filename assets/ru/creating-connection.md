...
- <details>
   ...
   ![Alt text](@images/binance_1.png)
    ...
    В аккаунте Binance и перейти в `Профиль` - `Управление API`. Нажмите `Создать API`.
    ...
    На платформе Viking используется средняя часть ключа. Например для поля `API Key`:
    ...
    Аналогично для `Secret` используется средняя часть из `PRIVATE KEY`
