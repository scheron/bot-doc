...
- <details>
   ...
   ![Alt text](@images/binance_1.png)
    ...
    Скопируйте ключи или сохраните ключи `Ed25519` в файлы `Private_key.txt` и `Public_key.txt`.
    ...
    На Viking используете серединную часть ключей. Например для поля `API Key`:
    ...
    Аналогично для `Secret` использовать серединную часть из `PRIVATE KEY`
