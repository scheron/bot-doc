...
**Важно:** так как у брокера присутствуют инструменты, в которых количество лотов может быть дробным, портфели будут создаваться с count = 1e-8 для инструментов с CTRADER. Так происходит для всех инструментов, где количество лотов может быть дробным. Это важно учитывать при настройках портфеля, в частности, при определении значения параметра count. К примеру, если указать в поле Count значение 1 000 000 000 (в примере выставляем 1 единицу портфеля), то будет выставлена заявка с объёмом 10 лотов. Из расчёта 1е-8 * 1 000 000 000 = 10. Если хотим выставить заявку с объёмом 1 лот, можем указать 100 000 000 (1е-8 * 100 000 000 = 1).
