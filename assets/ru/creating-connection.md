...
**Важно:** на данном подключении отутствует проверка получения статусов бумаг для выставления заявок. Это значит, что при включенной торговле на портфеле, сразу будут выполняться попытки отправки заявок на биржу, не дожидаясь момента, когда с биржи по бумаге придёт статус "Торгуется". Для того, чтобы избежать ошибочных транзакций на портфелях, использующих данное подключение, ОБЯЗАТЕЛЬНО нужно настроить расписание, в соответствие с расписанием торгов на данной бирже.
...
Коды типов инструментов, которыми планируется торговать. По умолчанию указаны коды всех инструментов, кроме опционов. Список возможных вариантов кодов можно проверить у EXANTE:
`MRCXXX` - Cash, FOREX;
`EXXXXX` - Stocks;
`EUXXXX` - Funds;
`DBXXXX` - Bonds;
