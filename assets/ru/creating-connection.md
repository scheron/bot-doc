...
**Важно:** на данном подключении отcутствует проверка статусов бумаг при выставлении заявок. При включенной торговле по портфелю, при наступлении условий для выставления заявки, проверка статуса бумаги не выполняется. Это может привести к тому, что заявка будет отправлена, когда бумага не торгуется. Чтобы этого не допустить рекомендуется настраивать расписание на портфелях, использующих данное подключение.
...
Коды типов инструментов, которыми планируется торговать. По умолчанию указаны коды всех инструментов, кроме опционов. Актуальный список вариантов кодов можно посмотреть в [документации EXANTE](https://api.exante.eu/fix-api/specification/#tag/Instrument-Identification):
`MRCXXX` - Cash, FOREX;
`EXXXXX` - Stocks;
`EUXXXX` - Funds;
`DBXXXX` - Bonds;
...
**Важно:** так как у брокера присутствуют инструменты, в которых количество лотов может быть дробным, портфели будут создаваться с [Сount](params-description.md#s.count) = 1e-8 для инструментов с CTRADER. Так происходит для всех инструментов, где количество лотов может быть дробным. Это важно учитывать при настройках портфеля, в частности, при определении значения параметра [Count](params-description.md#s.count). К примеру, если указать в поле [Count](params-description.md#s.count) значение 1 000 000 000 (в примере выставляем 1 единицу портфеля), то будет выставлена заявка с объёмом 10 лотов. Из расчёта $1е-8 \times 1 000 000 000 = 10$. Если хотим выставить заявку с объёмом 1 лот, можем указать 100 000 000 $(1е-8 \times 100 000 000 = 1)$.
