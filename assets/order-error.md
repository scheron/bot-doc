...
- **Ошибка:** `order adding error on *, error: REASON_UNDEFINED`<Anchor :ids="['error.undefined']" />
    ...
    **Описание:** Вы получите эту ошибку в том случае, если ошибка выставления заявки, которую присылает биржа, никак не классифицирована роботом. Торговля будет остановлена автоматически. В случае данной
    ...
    как `REASON_FLOOD` или `REASON_NO_MONEY`, то напишите в поддержку.

- **Ошибка:** `order adding error on *, continue trading, error: REASON_PRICE_OUT_OF_LIMIT`<Anchor :ids="['error.price_out_of_limit']" />

    **Описание:** Данная ошибка означает, что цена заявки вне лимита, и это нормально, так может быть, т.к. цены бумаг в некоторых случаях рассчитываются и зависят от ваших настроек. Возможно, вам следует поменять настройки.

- **Ошибка:** `order adding error on *, continue trading, error: REASON_CROSS`<Anchor :ids="['error.cross']" />

    **Описание:** Данная ошибка возникает когда вы пытаетесь торговать сами с собой или с другими роботами, торгующими на том же счёте и/или ИНН. При возникновении данной ошибки на инструменте первой ноги, заявка не выставится, робот продолжит работу по алгоритму.
