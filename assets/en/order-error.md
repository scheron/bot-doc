...
section: 9
summary: Categories of order submission, cancellation, and modification errors, what each reason means, and the exchange error code references.
---

# Order Submission, Cancellation, and Modification Errors <Anchor :ids="['errors']" />

## General Provisions <Anchor :ids="['errors.general']" />

During operation, the robot may encounter errors when placing, moving, or canceling orders. Such errors in themselves are not an indication of incorrect robot behavior. This is normal operation.
Errors displayed to the user may either be messages relayed from the exchange or the result of checks performed by the robot itself. The robot performs certain validations on its side—similar to those conducted by the exchange—without sending orders to the exchange that would inevitably be rejected.
If an order fails validation on the robot's side (and thus is not sent to the exchange), the error reason will have the postfix `LOCAL` added, for example, `REASON_FLOOD_LOCAL`.
All errors without the `LOCAL` postfix are responses from the exchange to an attempt to place, cancel, or modify an order; the robot simply relays these messages to the user.

Order cancellation errors most commonly occur when, after sending a cancellation request but before the exchange processes it, the order is filled in a trade.

## Error Categorization <Anchor :ids="['errors.categories']" />

All errors related to order submission, cancellation, and modification - whether received from the exchange or generated by the robot itself - are classified into several categories for easier handling (for example, on Moscow Exchange there are over ten different error codes, the essence of which boils down to the client not having sufficient funds to place an order). Below are the categories of order submission and modification errors.

- **Error:** `Order adding error on *, error: REASON_NO_MONEY`<Anchor :ids="['error.no_money']" />

    **Description:** Insufficient funds on the account. This error is always a response from the exchange; there is no version with the `LOCAL` postfix, as the robot does not monitor the client's balance nor checks whether sufficient funds are available for placing an order. This error category includes various exchange messages such as `Account has insufficient Available Balance`, `Value of position and orders exceeds position Risk Limit`, etc. The exact message or error code sent directly by the exchange can be found in the [Robot logs](interface.md#robot_logs) and [Robot logs history](interface.md#) widgets of the platform's graphical interface. For Moscow Exchange markets, you can view the exchange error codes [here](order-error.md#errors.codes). When this error occurs, check whether sufficient funds are available on the account for placing an order of the given volume for the financial instrument. If the account balance is sufficient, contact your senior trader, broker, or the exchange for clarification. Upon receiving this error, trading will be stopped automatically.

- **Error:** `order adding error on *, continue trading, error: REASON_FLOOD`<Anchor :ids="['error.flood']" />

    **Description:** Flood control — the maximum allowed number of transactions (orders to place, modify, or cancel orders) per unit of time for the given login has been exceeded, or the exchange has returned a message such as `Exchange is overloaded, try later`, or other similar messages indicating that the exchange is experiencing temporary difficulties but is still operational. For this robot, performing 30 transactions per second (the transaction activity unit currently used by Moscow Exchange) is trivial. This error may occur both with and without the `LOCAL` postfix.
	It should be understood that the robot accumulates a queue of transaction messages before sending them to the website log. There is a certain time lag between an event and its appearance in the log.
	The presence of flood errors is generally an indicator of excessive activity. This is what you should focus on, as it may prevent trading at the "right moment" if the login's capacity is exhausted by non-productive transactions at every instant.

- **Error:** `order adding error on *, error: REASON_UNDEFINED`<Anchor :ids="['error.undefined']" />

    **Description:** You will receive this error when the exchange returns an order submission error that is not classified by the robot. This error is always a response from the exchange; there is no `LOCAL` variant due to the very definition of this error category. In case of this error, the message sent by the exchange appears in the GUI log—please read it carefully. If you believe this message should be specifically handled by the robot (e.g., as `REASON_FLOOD` or `REASON_NO_MONEY`), please contact support. Upon receiving this error, trading will be stopped automatically.

- **Error:** `order adding error on *, continue trading, error: REASON_PRICE_WRONG`<Anchor :ids="['error.price_wrong']" />

    **Description:**This error generally indicates an invalid price specified in the order. This category includes orders with prices outside the exchange-defined price limits, as well as orders with the [Only maker](params-description.md#p.maker) flag set at a price matching the opposite side of the order book. Prices of financial instruments are sometimes calculated based on your settings, so upon encountering this error, adjusting portfolio settings may be necessary. This error may occur both with and without the `LOCAL` postfix. Local validation only checks price limits; verifying whether an order would remain passive after submission cannot be reliably performed on the robot’s side.

- **Error:** `order adding error on *, continue trading, error: REASON_CROSS`<Anchor :ids="['error.cross']" />

    **Description:**  This error occurs when you attempt to trade with yourself or with other traders/robots operating on the same account and/or INN (tax identification number). This error is always a response from the exchange; there is no `LOCAL` variant due to the nature of this error category. If this error occurs on the first-leg instrument, the order will not be placed, and the robot will continue operating according to its algorithm.
	If this error occurs on second-leg instruments, the robot will attempt to re-submit the order at a new price, shifting one tick deeper into its own side of the order book. Within a single order modification, up to 100 attempts resulting in `REASON_CROSS` errors are allowed. If this limit is reached, the order will not be placed.

- **Error:** `order adding error on *, continue trading, error: REASON_ZERO_AMOUNT_TO_MULTIPLE`<Anchor :ids="['error.zero_amount_to_multiple']" />

    **Description:** On certain exchanges (e.g., Deribit), the order size must be a multiple of the lot size. Before submitting an order, the size is rounded down to the nearest whole number of lots. If the intended order size is smaller than one lot, the rounded size becomes zero. The robot detects this and returns the error. To avoid such errors, you can set the [Overlay](/docs/params-description.html#p.overlay) parameter to one or higher. This error only occurs with the `LOCAL` postfix.
    
- **Error:** `order adding error on *, continue trading, error: REASON_NO_SESSION`<Anchor :ids="['error.no_session']" />

    **Error:** This error means that the trading session for the instrument is not active. This error may occur both with and without the `LOCAL` postfix.

- **Error:** `order adding error on *, error: REASON_TEMPORARY_PROHIBITED_LOCAL, original err_code=666666`<Anchor :ids="['error.temporary_prohibited']" />

    **Description:** This error, always accompanied by `err_code=666666`, indicates that order submission is temporarily unavailable because the daily limit of virtual trades executed by the robot has been exceeded.

**Important!** 

If the log contains `Stop trading!!!` along with an order submission error, and the error occurred on a financial instrument that is not marked as [Is first](params-description.md#s.is_first), then behavior equivalent to a `hard stop` is activated - i.e., the robot will attempt to cancel all existing orders in the portfolio and will not submit any new orders for any financial instruments.


## Error Codes <Anchor :ids="['errors.codes']" />

### MOEX FORTS

The up-to-date reference of error codes for the Futures Market is publicly available on the Moscow Exchange FTP. You can review it [here](http://ftp.moex.ru/pub/TWIME/Spectra/prod/doc/spectra_twime_ru.pdf)
...
The up-to-date reference of error codes for the Stock and Currency markets is publicly available on the Moscow Exchange FTP. You can review it [here](https://ftp.moex.com/pub/ClientsAPI/ASTS/docs/asts_message_list.csv)
