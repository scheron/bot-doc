...
Робот умеет уведомлять пользователя о наступлении определенных событий. Например, при резком падении финансового результата. Так как понятие "резкое падение" сильно зависит от выбранной стратегии и настроек портфеля, то для каждого портфеля настройки уведомлений задаются отдельно.
Кроме чисто сигнальной функции все уведомления могут выключать торговлю по соответствующему портфелю. При наступлении соответствующего события, уведомление появляется на сайте, а так же в [Telegram-бот](getting-started.md#тelegram-бот), если он подключен и настроен.
Следует обратить внимание на то, что большинство уведомлений работают только при включенной торговле на портфеле. Также существуют уведомления, которые настраиваются для конкретного транзакционного подключения к бирже, они описаны в разделе [Параметры позиций](params-description.md#параметры-позиций).
...
2. - Если в течение периода времени `Time (sec)` секунд значение выбранного параметра изменится на величину строго большую, чем заданный лимит, то сработает уведомление и алгоритм перейдет к пункту `1`.
...
Таким образом пользователь получит уведомление если за время не более чем `Time (sec)` значение заданного параметра изменилось строго больше, чем на `Value`.
...
- `Time (sec)` - период времени в секундах за которое считается падение;
...
**Важно!** Необходимо учитывать, что при отсутствии цен по бумагам портфеля и/или при невозможности посчитать [Sell/Buy](params-description.md#p.sell), значение параметров [Sell/Buy](params-description.md#p.sell) будет равно `0`.
...
Уведомлять о слишком большом количестве активных заявок по не [Is first](params-description.md#s.is_first) инструментам портфеля. Т.е. уведомление будет отправлено если максимум из числа активных заявок по
каждому из не [Is first](params-description.md#s.is_first) инструментов портфеля строго больше заданного числа процентов от [Max not hedged](params-description.md#p.max_not_hedged).
