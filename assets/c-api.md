...
- Все строки в тексте кода формулы должны быть валидными `UTF-8` строками, все лимиты длин строковых значений полей портфеля, бумаги и т.д. указаны в данной документации и в шаблонах в байтах;
...
**Важно!** Если значение поля какой-либо бумаги еще не было получено с биржи или для бумаги с данной биржи нельзя получить значение этого поля, то вы получите `0`. Нужно понимать что, например, на пустом стакане вы можете получить `0` в качестве цены бида или оффера,
...
- Если используются лучшие цены на покупку/продажу бумаг, то рекомендуется проверять [есть ли эти цены на самом деле](#bid-offer-check) (т.е. цена бумаги и объем этой цены должны быть отличны от нуля)
...
- Если используются лучшие цены на покупку/продажу бумаг и/или торговые стаканы, то рекомендуется проверять что лучшая цена покупки строго меньше лучшей цены продажи (для некоторых бирж нормально
...
| int trading_status()<Anchor hide :ids="['trading-status-check']" />            | статус торгуемости бумаги на бирже (битовая маска, возможные взведенные биты [TRADING_CAN_PLACE](#__trading_can_place__) и [TRADING_CAN_CANCEL](#__trading_can_cancel__), [пример](#__sec_status_check__)) |
...
| Метод                  | Описание                                                                                                                                                         |
|------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| bool is_available()<Anchor hide :ids="['order-book-check']" />    | доступен ли в данный момент для данной бумаги стакан                                                                                                             |
...
| Функция                                                                                | Описание                                                                                 |
|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| struct security_fields get_security_fields(const std::string& p, const std::string& s) | получить бумагу портфеля с именем p с [SecKey](params-description.md#s.sec_key) s |
| struct security_fields get_security_fields()                                           | получить главную бумагу текущего портфеля                                                |
| struct security_fields get_security_fields(const std::string& s)                       | получить бумагу текущего портфеля с [SecKey](params-description.md#s.sec_key) s   |
...
| Метод                                                       | Описание                                                                                                 |
|-------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| void restart_sec_iter()                                     | инциализировать итератор (для одного портфеля НЕЛЬЗЯ использовать один итератор вложенный в другой, порядок бумаг при итерировании НЕ определен)      |
| bool has_next_sec()                                         | есть ли еще бумаги в списке бумаг портфеля                                                               |
| security_fields next_sec()                                  | получить следующую бумагу портфеля ([ пример итерации ](#__Example3__)), порядок бумаг при итерировании НЕ определен (он не зависит от порядка добавления бумаг в портфель, настроек этих бумаг и т.п.) |
...
| <Anchor hide :ids="['__market_data_bests_online__']"/>MARKET_DATA_BESTS_ONLINE | int         | 1, для данной бумаги маркет-дата подключение с лучшими ценами на покупку/продажу онлайн|
| <Anchor hide :ids="['__market_data_ob_online__']"/>MARKET_DATA_OB_ONLINE | int         | 2, для данной бумаги маркет-дата подключение со стаканами онлайн |
...
<Anchor hide :ids="['__Example1__']"/>Чтобы получить, например, цену последней сделки для бумаги с СПб биржи (получение цены последней сделки по ключу возможно только для СПБ биржи):
...
<Anchor hide :ids="['__Example3__']"/>Чтобы пройти в цикле по всем бумагам портфеля и вывести имена бумаг в лог:
...
<Anchor hide :ids="['__Example9__']"/>Чтобы найти, например, сумму объемов активных заявок портфеля по бумаге по заданной цене 100:
...
Пусть имеется портфель с именем "test" и в этом портфеле есть два инструмента: фьючерс на индекс РТС "RIH6" и его опцион `call` на страйк 70000 "RI70000BB6", позиция по обоим бумагам в портфеле равна 1, а направление торговли у обоих [On_by](params-description.md#s.on_buy) = [Buy](params-description.md#p.buy).
