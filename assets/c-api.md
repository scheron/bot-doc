...
- Максимальная разрешенная длина каждого из полей формул в портфеле в данный момент составляет `12000` байт (это значение можно получить в [шаблоне портфеля](https://fkviking.github.io/bot-doc/docs/api.html#get_template_id)).
...
возможность небольшого расширения алгоритма работы робота пользователем, если же вам необходимо что-то намного более значительное, то для этих целей рекомендуется использовать [WebSocket API](https://fkviking.github.io/bot-doc/docs/api.html#api);
